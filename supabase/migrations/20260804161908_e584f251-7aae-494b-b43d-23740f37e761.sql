...
revoke all on function public.submeter_relato_mostra(uuid, text, categoria_relato, modo_participacao, text, text, text[], origem_imagem, boolean, boolean, boolean, jsonb) from public;
grant execute on function public.submeter_relato_mostra(uuid, text, categoria_relato, modo_participacao, text, text, text[], origem_imagem, boolean, boolean, boolean, jsonb) to anon, authenticated, service_role;

drop policy if exists relatos_mostra_insert_publico on public.relatos_mostra;
drop policy if exists coautores_insert_publico on public.coautores;
revoke insert on public.relatos_mostra from anon, authenticated;
revoke insert on public.coautores from anon, authenticated;